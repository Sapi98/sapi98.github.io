const publications = [
  {
    ...
    "bibtex": "@incollection{biswasdeep2022,\n  title={Deep Learning Algorithms for Efficient Analysis of ECG Signals to Detect Heart Disorders},\n  author={S. Dey, R. Pal, S. Biswas},\n  year={2022}\n}"
  },
  {
    "type": "journal",
  ...
  },
  {
    ...
    "bibtex": "@article{biswaslearning2025,\n  title={Learning Rate Tuner with Relative Adaptation (LRT-RA): Road to Sustainable Computing},\n  author={S. Biswas, S. Dey, S. Nath},\n  year={2025}\n}"
  },
  {
    "type": "journal",
    "typeLabel": "Journal Papers",
  ...
  },
  {
    ...
    "bibtex": "@article{biswasextended2020,\n  title={Extended Nucleic Acid Memory as the Future of Data Storage Technology},\n  author={S. Biswas, S. Nath, J. K. Sing, S. K. Sarkar},\n  year={2020}\n}"
  },
  {
    "type": "conference",
    "typeLabel": "Conference Papers",
    "year": 2025,
    ...
    "bibtex": "@inproceedings{biswascomputability2025,\n  title={Computability of Real Functions with Oracle Pointer Machines implies Real-Time simulation of Chemical Reaction Networks},\n  author={S. Biswas},\n  year={2025}\n}"
  },
  {
    "type": "conference",
    "typeLabel": "Conference Papers",
    ...
    "bibtex": "@inproceedings{biswasselective2022,\n  title={Selective Run-Length Encoding on Extended Nucleic Acid Memory},\n  author={S. Biswas, T. Ghosh, S. Nath},\n  year={2022}\n}"
  },
  {
    "type": "conference",
    "typeLabel": "Conference Papers",
  ...
  },
  {
    ...
    "bibtex": "@inproceedings{biswasstoring2019,\n  title={Storing Digital Data in Nucleic Acid Memory with Extended Genetic Alphabet},\n  author={S. Biswas, S. Nath, J. K. Sing, S. K. Sarkar},\n  year={2019}\n}"
  },
  {
    "type": "thesis",
    "typeLabel": "Thesis and Dissertations",
    "year": 2025,
    ...
    "bibtex": "@mastersthesis{biswasoracle2025,\n  title={Oracle Pointer Machines},\n  author={S. Biswas},\n  year={2025}\n}"
  },
  {
    "type": "preprint",
    "typeLabel": "Preprints",
  ...
  },
  {
    ...
    "bibtex": "@misc{biswasmagnocluster2026,\n  title={MagnoCluster: A Physics-Informed Unsupervised Learning Framework for Interpretable Classification of Magnetocaloric Materials},\n  author={S. Biswas, A. Biswas, Y. Mudryk, P. Singh},\n  year={2026}\n}"
  },
  {
    "type": "preprint",
    "typeLabel": "Preprints",
    ...
    "bibtex": "@misc{biswasrelative2023,\n  title={Relative Learning Rate Adaptation on Loss Feedback},\n  author={S. Biswas, S. Dey},\n  year={2023}\n}"
  }
];
const categoryOrder=["book","journal","conference","thesis","preprint"];
const categoryLabels={book:"Book Chapter",journal:"Journal Papers",conference:"Conference Papers",thesis:"Thesis and Dissertations",preprint:"Preprints"};
function sortPubs(pubs){return [...pubs].sort((a,b)=>(b.year-a.year)||a.title.localeCompare(b.title));}
function renderAllSummary(){return `<section class="pub-total-block"><div><p class="eyebrow">All categories</p><h3>All Publications</h3></div><span>${publications.length} total publications</span></section>`;}
function renderCard(pub,index){const view=pub.link&&pub.link!=="#"?`<a class="btn btn-small btn-soft" href="${pub.link}" target="_blank" rel="noopener">View</a>`:"";return `<article class="publication-card" data-type="${pub.type}"><div class="pub-topline"><span class="pub-number">${index}</span><span class="pub-meta">${categoryLabels[pub.type]}</span></div><h3>${pub.title}</h3><p class="pub-authors">${pub.authors}</p><p class="pub-venue">${pub.venue}</p><div class="pub-actions">${view}<button class="btn btn-small btn-soft bib-toggle" type="button">BibTeX</button></div><pre class="bibtex" hidden>${pub.bibtex||""}</pre></article>`;}
function renderCategory(type,pubs){const sorted=sortPubs(pubs);return `<section class="pub-category-block" data-category="${type}"><div class="pub-category-heading"><h3>${categoryLabels[type]}</h3><span>${sorted.length} total</span></div><div class="publication-list nested">${sorted.map((pub,i)=>renderCard(pub,i+1)).join("")}</div></section>`;}
function renderPublications(filter="all"){const c=document.getElementById("publicationList");if(!c)return;c.innerHTML=filter==="all"?renderAllSummary()+categoryOrder.map(t=>renderCategory(t,publications.filter(p=>p.type===t))).join(""):renderCategory(filter,publications.filter(p=>p.type===filter));setupBibToggles();}
function setupPublicationFilters(){document.querySelectorAll(".filter-btn").forEach(b=>b.addEventListener("click",()=>{document.querySelectorAll(".filter-btn").forEach(x=>x.classList.remove("active"));b.classList.add("active");renderPublications(b.dataset.filter);}));}
function setupBibToggles(){document.querySelectorAll(".bib-toggle").forEach(b=>{b.onclick=()=>{const pre=b.closest(".publication-card").querySelector(".bibtex");pre.hidden=!pre.hidden;b.textContent=pre.hidden?"BibTeX":"Hide BibTeX";};});}
document.addEventListener("DOMContentLoaded",()=>{renderPublications();setupPublicationFilters();});
