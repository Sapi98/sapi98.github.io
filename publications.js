...
function renderAllSummary() {
  return `<section class="pub-total-block"><div><p class="eyebrow">All categories</p><h3>All Publications</h3></div><span>${publications.length} total publications</span></section>`;
}

function renderCard(pub, index) {
  const view = pub.link && pub.link !== "#" ? `<a class="btn btn-small btn-soft" href="${pub.link}" target="_blank" rel="noopener">View</a>` : "";
  return `<article class="publication-card" data-type="${pub.type}"><div class="pub-topline"><span class="pub-number">${index}</span><span class="pub-meta">${categoryLabels[pub.type]}</span></div><h3>${pub.title}</h3><p class="pub-authors">${pub.authors}</p><p class="pub-venue">${pub.venue}</p><div class="pub-actions">${view}<button class="btn btn-small btn-soft bib-toggle" type="button">BibTeX</button></div><pre class="bibtex" hidden>${pub.bibtex||""}</pre></article>`;
}

function renderCategory(type, pubs) {
  const sorted = sortPubs(pubs);
  return `<section class="pub-category-block" data-category="${type}"><div class="pub-category-heading"><h3>${categoryLabels[type]}</h3><span>${sorted.length} total</span></div><div class="publication-list nested">${sorted.map((pub,i)=>renderCard(pub,i+1)).join("")}</div></section>`;
}

function renderPublications(filter = "all") {
  const c = document.getElementById("publicationList");
  if (!c) return;
  c.innerHTML = filter === "all" ? renderAllSummary() + categoryOrder.map(t => renderCategory(t, publications.filter(p => p.type === t))).join("") : renderCategory(filter, publications.filter(p => p.type === filter));
  setupBibToggles();
}
...
function setupBibToggles() {
  document.querySelectorAll(".bib-toggle").forEach(b => {
    b.onclick = () => {
      const pre = b.closest(".publication-card").querySelector(".bibtex");
      pre.hidden = !pre.hidden;
      b.textContent = pre.hidden ? "BibTeX" : "Hide BibTeX";
    };
  });
}
